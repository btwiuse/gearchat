...
function Login() {
  ...
  const signUp = async (event) => {
    ...
    try {
      ...
      if (image) {
        const uploadedImageSnapshot = await storage.ref(
          `users/${result.user.uid}${image.name}`,
        ).put(image);
    ...
    <div className="login">
      <div className="login__container">
        ...
        <div className="login__text">
          <h1>Sign in to GearChat made by Practical Dev</h1>
        </div>
        <div>
          ...
          <div style={modalStyle} className={classes.paper}>
            <form className="login__signup">
              ...
              <Input
                ...
                onChange={(e) => setPassword(e.target.value)}
              />
              <InputLabel htmlFor="image" variant="filled" shrink={true}>
